    ...
    public class ViveFoveatedRendering : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            if (foveatedRenderingInited)
            {
                ...
                ViveFoveatedGazeUpdater.AttachGazeUpdater(gameObject);

                SetPatternPreset(ShadingPatternPreset.SHADING_PATTERN_CUSTOM);
                SetRegionRadii(TargetArea.INNER, new Vector2(0.25f, 0.25f));
                SetRegionRadii(TargetArea.MIDDLE, new Vector2(0.33f, 0.33f));
                SetRegionRadii(TargetArea.PERIPHERAL, new Vector2(2.0f, 2.0f));

                SetShadingRatePreset(ShadingRatePreset.SHADING_RATE_CUSTOM);
                SetShadingRate(TargetArea.INNER, ShadingRate.X1_PER_PIXEL);
                SetShadingRate(TargetArea.MIDDLE, ShadingRate.X1_PER_1X2_PIXELS);
                SetShadingRate(TargetArea.PERIPHERAL, ShadingRate.X1_PER_2X2_PIXELS);
